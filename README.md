...
3. **Note cleaning & quantization** — filters noise, merges fragments, snaps to tempo grid
4. **Music theory analysis** — detects key, scale, tempo, and chord progressions
5. **Instrument mapping** — maps notes to real playable positions on the target instrument (frets, strings, etc.)
6. **Output generation** — produces tabs, sheet music, chord sheets, fretboard diagrams, and a synthesized audio preview
...
| 4 | Tempo detection & quantization to 16th-note grid |
| 5 | Key / scale detection (Krumhansl-Schmuckler algorithm) |
| 6 | Instrument mapping (guitar string + fret assignment) |
| 7 | Chord detection (template matching, key-aware) |
| 8 | Tab generation (ASCII guitar tabs) |
...
| Music theory | Custom Krumhansl-Schmuckler implementation |
